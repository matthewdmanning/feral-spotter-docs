...
- `useBoundingBoxStore` already keys boxes by `${cat_id}:${photo_local_id}` independent of whether that `cat_id` has a matching `ObservedCat` — this is *why* "first box declares a cat" and "mid-pass abandonment keeps data" both required zero schema change.
